...
//to a simpler expression.nThe string has a length greater or equal to one and contains only letters from ato z.

function printerError(s) {
    let longitud = s.length;
    let Ones = s.replace(/[aA-mM]/g, "").length;
    console.log(longitud, Ones)
    let result = String(Ones + "/" + longitud)
    return result
}